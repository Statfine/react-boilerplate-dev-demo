...
class Transformable extends PureComponent {
  ...
  componentDidMount() {
    ...
    drag.style.left = defaultPosition.x && `${Math.round(defaultPosition.x)}px`;
    drag.style.top = defaultPosition.y && `${Math.round(defaultPosition.y)}px`;
    drag.style.transform = defaultPosition.rotate ? `rotate(${defaultPosition.rotate}deg)` : `rotate(${0}deg)`;
    if (this.props.translate) {
      drag.style.width = defaultPosition.w ? `${defaultPosition.w}px` : '100px';
      drag.style.height = defaultPosition.h ? `${defaultPosition.h}px` : '100px';
  ...
  handleMouseMove = (ev) => {
    ...
    if (dragFlag === 'nw') {
      if (
        (moveX < 0 && moveX + start.startX >= 0) ||
        (moveX > 0 && start.startW - moveX >= 30)
      ) {
        this.trancontainer.style.width = `${start.startW - moveX}px`;
        this.trancontainer.style.left = `${start.startL + moveX}px`;
      }
      if (
        (moveY < 0 && moveY + start.startY >= 0) ||
        (moveY > 0 && start.startH - moveY >= 30)
      ) {
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {!disabled && translate &&
          <ContrlArrow>
            <ArrowLeftTop onMouseDown={(e) => this.handleMouseDown(e, 'nw')} />
            <ArrowRightTop onMouseDown={(ev) => this.handleMouseDown(ev, 'ne')} />
            <ArrowBottomLeft onMouseDown={(ev) => this.handleMouseDown(ev, 'sw')} />
            <ArrowBottomRight onMouseDown={(ev) => this.handleMouseDown(ev, 'se')} />
            <ArrowLeft onMouseDown={(e) => this.handleMouseDown(e, 'w')} />
            <ArrowRight onMouseDown={(e) => this.handleMouseDown(e, 'e')} />
            <ArrowTop onMouseDown={(e) => this.handleMouseDown(e, 'n')} />
            <ArrowBottom onMouseDown={(e) => this.handleMouseDown(e, 's')} />
          </ContrlArrow>
        }
